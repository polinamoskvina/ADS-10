...
#include <iostream>
#include <vector>
std::vector<char> temporary_delete(std::vector<char> num, int ind_to_delete);
struct Node {
...
class Tree {
 ...
 public:
    ...
    explicit Tree(std::vector<char> in): size(0) {
        ...
        for (int i = 0; i < in.size(); i++) {
            std::vector<char> temporary = in;
            temporary = temporary_delete(temporary, i);
            add_childrens_for_child(temporary, root->children[i]);
        }
    }
    ...
    void add_childrens_for_child(std::vector<char> iner, Node* parenti) {
        ...
        for (int i = 0; i < iner.size(); i++) {
            ...
            std::vector<char> temporary = iner;
            temporary = temporary_delete(temporary, i);
            add_childrens_for_child(temporary, newNode);
        }
    }
